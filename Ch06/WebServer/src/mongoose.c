 ...
 * All rights reserved
 *
 * 2015-12-11 GZ Modified mg_send_http_file() and send_http_error()
 */
...
//GZ    mg_printf(nc, "%s", "HTTP/1.1 404 Not Found\r\nContent-Length: 0\r\n\r\n");
    char *pa = strstr(path, "/ROOT");
    if (pa == NULL) pa = path;
    else pa += 5;
    mg_printf(nc, "%s%d%s%s%s", "HTTP/1.1 404 Not Found\r\n"
        "Content-Type: text/plain;charset=utf-8\r\n"
        "Content-Length: ", strlen("HTTP/1.1 404 Not Found: ") + strlen(pa),
